...
export const useSocketInit = (token: string | null, userType: 'CUSTOMER' | 'STAFF') => {
  useEffect(() => {
    if (!token) return
...
const handleNotificationClick = (notif) => {
  switch (notif.type) {
    ...
    case 'COMPLETE_INVOICE':
      return navigate(`/invoices/${notif.metadata.invoiceId}/assign-delivery`)
  }
}
```
...
```typescript
type NotificationType = 'INVOICE_CREATE' | 'ASSIGN_ORDER' | 'ASSIGN_INVOICE' | 'COMPLETE_INVOICE'
