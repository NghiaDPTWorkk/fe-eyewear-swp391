...
export const VoucherAddition: React.FC<VoucherAdditionProps> = ({
  ...
  const filteredFormCustomers = customers.filter(
    (c) =>
      !custSearch ||
    ...
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <Section title="Voucher Identity">
              ...
              <div className="col-span-4">
                <FormRow label="Voucher Code *">
                  <input
                    ...
                    className={`${inputCls} font-mono font-bold tracking-wider placeholder:font-sans placeholder:font-normal`}
            ...
            <FormRow label="Internal Description">
              <textarea
                ...
                className={`${inputCls} min-h-[80px] resize-none py-2.5`}
                placeholder="What is this voucher for? (internal note)"
              />
            </FormRow>
          </Section>
            ...
            <div className="grid grid-cols-2 gap-5">
              <FormRow label="Discount Engine">
                <div className="flex p-1 bg-neutral-100 rounded-xl border border-neutral-200/50 text-[10px] font-black uppercase tracking-tighter">
                  <button
                    onClick={() => setForm({ ...form, typeDiscount: 'PERCENTAGE' })}
                  ...
                  <button
                    onClick={() => setForm({ ...form, typeDiscount: 'FIXED' })}
              ...
              <FormRow
                label={
                  form.typeDiscount === 'PERCENTAGE' ? 'Discount Value (%)' : 'Discount Value (₫)'
                }
              >
                <input
                  type="number"
                  ...
                  className={`${inputCls} font-bold text-mint-600 focus:ring-mint-400/30`}
                  placeholder="0"
                />
              </FormRow>
            </div>
              ...
              <FormRow label="Application Scope">
                <select
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      applyScope: e.target.value as VoucherApplyScope,
                      assignedTo: []
                    })
                  }
                  className={inputCls}
            ...
            {!form.startedDate || !form.endedDate ? (
              <div className="mt-3 p-3 bg-amber-50 border border-amber-100/50 rounded-xl flex items-center gap-3">
                ...
                <p className="text-[11px] font-bold text-amber-700 uppercase tracking-tight">
                  Status will be live calculated
                </p>
              </div>
            ) : (
            ...
            <Section
              ...
              action={
                <div className="flex bg-neutral-100 p-0.5 rounded-lg border border-neutral-200/50">
                  <button
                    onClick={() => setForm({ ...form, selectionMode: 'RULE' })}
                    className={`px-3 py-1 text-[10px] font-black rounded-md transition ${form.selectionMode === 'RULE' ? 'bg-white text-mint-600 shadow-sm' : 'text-neutral-400'}`}
                  >
                    SEGMENT
                  </button>
                  <button
                    onClick={() => setForm({ ...form, selectionMode: 'MANUAL' })}
                    className={`px-3 py-1 text-[10px] font-black rounded-md transition ${form.selectionMode === 'MANUAL' ? 'bg-white text-mint-600 shadow-sm' : 'text-neutral-400'}`}
                  >
                    SELECT
                  </button>
                </div>
              ...
              {form.selectionMode === 'RULE' ? (
                <div className="space-y-4 pt-1">
                  <div className="grid grid-cols-2 gap-4">
                    <FormRow label="Min Spending (₫)">
                      <input
                        type="number"
                        value={form.minSpendingRule}
                        onChange={(e) => setForm({ ...form, minSpendingRule: +e.target.value })}
                        className={inputCls}
                        placeholder="0"
                      />
                    </FormRow>
                    <FormRow label="Min Orders">
                      <input
                        type="number"
                        value={form.minOrderRule}
                        onChange={(e) => setForm({ ...form, minOrderRule: +e.target.value })}
                        className={inputCls}
                        placeholder="0"
                      />
                    </FormRow>
                  ...
                  <div className="p-4 bg-gradient-to-br from-mint-50 to-white rounded-2xl border border-mint-100 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-black text-mint-900 tracking-tight">
                          Active Reach
                        </p>
                        <p className="text-[11px] text-mint-600/60 font-bold uppercase">
                          Dynamic matching
                        </p>
                      </div>
                    </div>
                      ...
                      <span className="text-3xl font-black text-mint-700 tabular-nums">
                        {
                          customers.filter(
                            (c) =>
                              c.totalSpending >= form.minSpendingRule &&
                              c.orderCount >= form.minOrderRule
                          ).length
                        }
                      </span>
                ...
                <div className="space-y-4 pt-1">
                  <div className="relative group">
                    <IoSearchOutline
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400 group-focus-within:text-mint-500 transition-colors"
                      size={16}
                    />
                    <input
                      value={custSearch}
                  ...
                  <div className="grid grid-cols-2 gap-3 max-h-56 overflow-y-auto pr-1 custom-scrollbar">
                    {filteredFormCustomers.map((c) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black transition-colors ${isSel ? 'bg-mint-500 text-white shadow-lg' : 'bg-neutral-100 text-neutral-400'}`}
                          >
                            {c.avatarInitials}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-neutral-400 font-black uppercase mt-0.5">
                              ₫{c.totalSpending.toLocaleString()}
                            </p>
                          </div>
                          <div
                            className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${isSel ? 'bg-emerald-500 border-emerald-500 scale-110' : 'border-neutral-200'}`}
                          >
        ...
        <div className="flex justify-end gap-3 px-8 py-5 border-t border-neutral-100 bg-neutral-50/50 flex-shrink-0">
          <button
            onClick={onClose}
            className="px-6 py-2.5 rounded-xl border border-neutral-200 text-neutral-500 text-[11px] font-black uppercase tracking-widest hover:bg-white hover:text-neutral-700 transition shadow-sm"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="px-10 py-2.5 rounded-xl bg-gradient-to-r from-mint-500 to-mint-600 text-white text-[11px] font-black uppercase tracking-widest hover:from-mint-600 hover:to-mint-700 transition shadow-xl shadow-mint-200/50 disabled:opacity-40 disabled:shadow-none flex items-center gap-2"
