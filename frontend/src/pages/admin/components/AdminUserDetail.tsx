              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 group">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-700">{formatDate(safeUser.createdAt || safeUser.created_at)}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              {safeUser.address && safeUser.address.length > 0 ? (
                <div className="space-y-3">
                  {safeUser.address.map((addr: any, idx: number) => (
                    <div key={idx} className="p-4 bg-neutral-50/50 rounded-2xl border border-neutral-100 space-y-1">
                      <div className="flex items-center gap-2 mb-1">
                        <IoLocationOutline className="text-mint-600" size={14} />
                        <span className="text-[10px] font-bold text-neutral-400 uppercase">Address #{idx + 1}</span>
                      </div>
                      <p className="text-xs font-semibold text-gray-700">{addr.street}</p>
                      <p className="text-[11px] text-neutral-500">{addr.ward}, {addr.city}</p>
                    </div>
                  ))}
                </div>
              ) : (
