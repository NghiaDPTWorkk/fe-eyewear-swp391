...
const mkCond = (id: string, min: number, val: number, max: number | null): DiscountCondition =>
  ({ id, minOrderValue: min, discountValue: val, maxDiscount: max })

const INITIAL_VOUCHERS: Voucher[] = [
  {
    id: 'v-001', code: 'SUMMER25', description: 'Summer sale – multi-tier discount',
    discountType: 'PERCENTAGE', scope: 'ALL',
    conditions: [
      mkCond('c1', 9000000, 10, 500000),
      mkCond('c2', 1000000, 2,  90000)
    ],
    maxUsage: 100, usedCount: 42, status: 'ACTIVE',
    startDate: '2026-03-01', endDate: '2026-04-30', assignedTo: []
  },
  {
    id: 'v-002', code: 'NEWUSER100K', description: 'Welcome voucher – fixed 100k',
    discountType: 'FIXED', scope: 'SPECIFIC',
    conditions: [mkCond('c3', 300000, 100000, null)],
    maxUsage: 200, usedCount: 178, status: 'ACTIVE',
    startDate: '2026-01-01', endDate: '2026-12-31',
    assignedTo: ['cus-002', 'cus-004', 'cus-005']
  },
  {
    id: 'v-003', code: 'FLASH50', description: 'Flash sale – 50% off',
    discountType: 'PERCENTAGE', scope: 'ALL',
    conditions: [mkCond('c4', 1000000, 50, 300000)],
    maxUsage: 30, usedCount: 30, status: 'INACTIVE',
    startDate: '2026-02-14', endDate: '2026-02-14', assignedTo: []
  },
  {
    id: 'v-004', code: 'LOYAL200K', description: 'Loyalty reward – 200k fixed',
    discountType: 'FIXED', scope: 'SPECIFIC',
    conditions: [mkCond('c5', 800000, 200000, null)],
    maxUsage: 50, usedCount: 12, status: 'ACTIVE',
    startDate: '2026-03-05', endDate: '2026-06-30',
    assignedTo: ['cus-001', 'cus-006']
  },
  {
    id: 'v-005', code: 'EXPIRE10', description: 'Year-end promo – expired',
    discountType: 'PERCENTAGE', scope: 'ALL',
    conditions: [mkCond('c6', 0, 10, null)],
    maxUsage: 500, usedCount: 488, status: 'EXPIRED',
    startDate: '2025-12-01', endDate: '2025-12-31', assignedTo: []
  },
]
...
const EMPTY_FORM: FormState = {
  code: '', description: '', discountType: 'PERCENTAGE', scope: 'ALL',
  conditions: [{ id: newCondId(), minOrderValue: 0, discountValue: 0, maxDiscount: null }],
  maxUsage: 100, status: 'ACTIVE', startDate: '', endDate: '', assignedTo: []
}
...
export default function ManagerVouchersPage() {
  ...
  const filtered = vouchers.filter((v) => {
    ...
    return (statusFilter === 'all' || matchStatus) &&
  ...
  const openCreate = () => {
    setEditingId(null)
    setForm({ ...EMPTY_FORM, conditions: [{ id: newCondId(), minOrderValue: 0, discountValue: 0, maxDiscount: null }] })
    setCustSearch('')
    setShowForm(true)
  }
  ...
  const openEdit = (v: Voucher) => {
    ...
    setForm({
      ...
      scope: v.scope, conditions: v.conditions.map((c) => ({ ...c })),
      maxUsage: v.maxUsage, status: v.status,
      startDate: v.startDate, endDate: v.endDate, assignedTo: [...v.assignedTo]
    })
    setCustSearch('')
    setShowForm(true)
  }
  ...
  const addCondition = () =>
    setForm((f) => ({ ...f, conditions: [...f.conditions, { id: newCondId(), minOrderValue: 0, discountValue: 0, maxDiscount: null }] }))
  ...
  const updateCondition = (id: string, field: keyof DiscountCondition, value: number | null) =>
    setForm((f) => ({ ...f, conditions: f.conditions.map((c) => c.id === id ? { ...c, [field]: value } : c) }))
  ...
  const filteredFormCustomers = MOCK_CUSTOMERS.filter((c) =>
      ...
      <div className="bg-white rounded-xl border border-neutral-100 shadow-sm overflow-hidden">
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((v, idx) => {
            ...
            return (
              <div key={v.id} className={`grid grid-cols-12 gap-4 px-6 py-4 items-start hover:bg-mint-50/30 transition-colors ${idx !== filtered.length - 1 ? 'border-b border-neutral-50' : ''}`}>
                {/* Code */}
                <div className="col-span-2">
                  ...
                  <span className={`mt-1 inline-block text-[10px] font-bold px-1.5 py-0.5 rounded border ${v.scope === 'ALL' ? 'bg-neutral-100 text-neutral-500 border-neutral-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                    {v.scope === 'ALL' ? 'ALL USERS' : 'SPECIFIC'}
                  </span>
                </div>
                {/* Description */}
                <div className="col-span-3 text-sm text-neutral-600 truncate pt-0.5" title={v.description}>{v.description}</div>
                {/* Conditions */}
                <div className="col-span-3 space-y-1">
                  {v.conditions.map((c) => (
                    <p key={c.id} className="text-xs text-neutral-600 bg-neutral-50 border border-neutral-100 rounded px-2 py-1 leading-tight">
                ...
                <div className="col-span-1 flex flex-col items-center gap-1 pt-0.5">
                  <span className="text-xs font-semibold text-neutral-700">{v.usedCount}/{v.maxUsage}</span>
                  <div className="w-full h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${v.usedCount >= v.maxUsage ? 'bg-red-400' : 'bg-mint-400'}`}
                ...
                <div className="col-span-1 flex justify-center gap-1 pt-0.5">
                  ...
                  <button onClick={() => setDeleteId(v.id)} className="p-1.5 rounded-lg text-neutral-400 hover:text-red-500 hover:bg-red-50 transition" title="Delete"><IoTrashOutline size={16} /></button>
                </div>
              </div>
            )
          })
        ...
        <ModalOverlay onClose={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100 flex-shrink-0">
              <h2 className="text-lg font-bold text-neutral-900">{editingId ? 'Edit Voucher' : 'Create Voucher'}</h2>
              <button onClick={() => setShowForm(false)} className="p-2 rounded-full hover:bg-neutral-100 text-neutral-400 hover:text-neutral-700 transition"><IoCloseOutline size={20} /></button>
            </div>
            ...
            <div className="overflow-y-auto px-6 py-5 space-y-5 flex-1">
              ...
              <Section title="Basic Info">
                <div className="grid grid-cols-2 gap-4">
                  <FormRow label="Code *">
                    <input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })}
                      className={`${inputCls} font-mono`} placeholder="e.g. SUMMER25" />
                  </FormRow>
                  <FormRow label="Status">
                    <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as VoucherStatus })} className={inputCls}>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <FormRow label="Discount Type">
                    <select value={form.discountType} onChange={(e) => setForm({ ...form, discountType: e.target.value as DiscountType })} className={inputCls}>
                      <option value="PERCENTAGE">Percentage (%)</option>
                      <option value="FIXED">Fixed Amount (₫)</option>
                    </select>
                  </FormRow>
                  <FormRow label="Max Usage">
                    <input type="number" min={1} value={form.maxUsage} onChange={(e) => setForm({ ...form, maxUsage: +e.target.value })} className={inputCls} />
                  </FormRow>
                  <FormRow label="Scope">
                    <select value={form.scope} onChange={(e) => setForm({ ...form, scope: e.target.value as VoucherScope, assignedTo: [] })} className={inputCls}>
              ...
              {/* Discount Conditions */}
              <Section title="Discount Conditions"
                action={
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-12 gap-2 text-[10px] font-bold text-neutral-400 uppercase tracking-widest px-1">
                    ...
                    <div className="col-span-3">{form.discountType === 'PERCENTAGE' ? 'Discount (%)' : 'Discount (₫)'}</div>
                    <div className="col-span-4">Max Discount (₫) — optional</div>
                    <div className="col-span-1" />
                  </div>

                  {form.conditions.map((cond) => (
                    <div key={cond.id} className="grid grid-cols-12 gap-2 items-center bg-neutral-50 border border-neutral-100 rounded-lg p-2">
                      <div className="col-span-4">
                        <input type="number" min={0} value={cond.minOrderValue}
                          onChange={(e) => updateCondition(cond.id, 'minOrderValue', +e.target.value)}
                      ...
                      <div className="col-span-3">
                        <input type="number" min={0} value={cond.discountValue}
                          onChange={(e) => updateCondition(cond.id, 'discountValue', +e.target.value)}
                          className="w-full px-2.5 py-1.5 rounded border border-neutral-200 text-sm text-neutral-800 focus:outline-none focus:ring-1 focus:ring-mint-400 bg-white" />
                      </div>
                      <div className="col-span-4">
                        <input type="number" min={0}
                          ...
                          onChange={(e) => updateCondition(cond.id, 'maxDiscount', e.target.value === '' ? null : +e.target.value)}
                      ...
                      <div className="col-span-1 flex justify-center">
                        {form.conditions.length > 1 && (
                          <button onClick={() => removeCondition(cond.id)} className="p-1 rounded text-neutral-300 hover:text-red-400 hover:bg-red-50 transition">
                ...
                {form.conditions.some((c) => c.discountValue > 0) && (
                  <div className="mt-3 p-3 bg-mint-50 border border-mint-200 rounded-lg space-y-1">
                    <p className="text-[10px] font-bold text-mint-700 uppercase tracking-widest mb-2">Preview</p>
                    {form.conditions
              ...
              {form.scope === 'SPECIFIC' && (
                <Section title="Assign to Customers"
                  badge={form.assignedTo.length > 0 ? `${form.assignedTo.length} selected` : undefined}>
                  <div className="relative mb-3">
                    <IoSearchOutline className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" size={15} />
                    <input value={custSearch} onChange={(e) => setCustSearch(e.target.value)}
                      placeholder="Search customer..." className="w-full pl-8 pr-4 py-2 rounded-lg border border-neutral-200 text-sm focus:outline-none focus:ring-2 focus:ring-mint-300/50 focus:border-mint-400 transition" />
                  </div>
                      ...
                      return (
                        <div key={c.id} onClick={() => toggleFormCustomer(c.id)}
                          ...
                          <div className="flex items-center gap-2.5">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${selected ? 'bg-mint-500 text-white' : 'bg-neutral-100 text-neutral-600'}`}>
                              {c.avatarInitials}
                            </div>
                            <div>
                          ...
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition ${selected ? 'bg-mint-500 border-mint-500' : 'border-neutral-300'}`}>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-neutral-100 flex-shrink-0">
              <button onClick={() => setShowForm(false)} className="px-5 py-2 rounded-lg border border-neutral-200 text-neutral-600 text-sm font-semibold hover:bg-neutral-50 transition">Cancel</button>
              <button onClick={handleSave} className="px-6 py-2 rounded-lg bg-mint-500 text-white text-sm font-bold hover:bg-mint-600 transition flex items-center gap-1.5 disabled:opacity-50"
                disabled={!form.code.trim() || form.conditions.length === 0}>
                <IoCheckmarkOutline size={16} />{editingId ? 'Save Changes' : 'Create Voucher'}
              </button>
            </div>
          </div>
        </ModalOverlay>
