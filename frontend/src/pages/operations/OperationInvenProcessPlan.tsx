...
import { usePreOrderImportDetail, useUpdatePreOrderImportStatus } from '@/features/operations/hooks/usePreOrderImports'
...
export default function OperationInvenProcessPlan() {
  ...
  const handleConfirmDone = async () => {
    if (!receiptId) return
    if (window.confirm('Are you sure you want to mark this batch as DONE? This action will update the inventory status.')) {
    ...
    return (
      <Container className="pt-10">
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
           ...
           <p className="text-sm text-slate-400 mt-1">The batch ID you provided does not exist or has been deleted.</p>
           <button
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
               <h1 className="text-2xl font-black text-slate-900 tracking-tight">Process Inventory Plan</h1>
               {detail && (
                 <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border shadow-sm ${STATUS_CONFIG[detail.status as keyof typeof STATUS_CONFIG]?.bg} ${STATUS_CONFIG[detail.status as keyof typeof STATUS_CONFIG]?.color}`}>
                   {detail.status}
                 </span>
               )}
            </div>
            <p className="text-sm text-slate-500 font-medium">Verification and confirmation of batch inventory status.</p>
          </div>
        </div>
        ...
        {detail && detail.status === 'PENDING' && (
          <button
            ...
            className="flex items-center gap-2 px-8 py-3 bg-mint-500 text-white rounded-2xl font-bold border-b-4 border-mint-700 shadow-xl shadow-mint-100 hover:bg-mint-600 hover:translate-y-[1px] hover:border-b-2 transition-all active:translate-y-[4px] active:border-b-0"
          >
            ...
            <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-8 relative overflow-hidden">
               ...
               <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-6 opacity-60">
                     ...
                     <span className="text-[10px] font-bold uppercase tracking-widest">Product Information</span>
                  </div>
                  <div className="grid grid-cols-2 gap-10">
                     <div className="space-y-1">
                        <p className="text-3xl font-black text-slate-900 font-mono tracking-tight">{detail.sku}</p>
                        <p className="text-sm text-slate-400 font-medium">{detail.description}</p>
                     </div>
                     <div className="flex flex-col items-end justify-center">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 text-right">Batch ID</p>
                        <p className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full">{detail._id}</p>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-8">
                 <div className="flex flex-col gap-1">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Expected Quantity</p>
                    <div className="flex items-baseline gap-2">
                       <span className="text-4xl font-black text-slate-800">{detail.targetQuantity}</span>
              ...
              <div className="bg-mint-50 rounded-[32px] border border-mint-100 shadow-sm p-8">
                 <div className="flex flex-col gap-1">
                    <p className="text-[10px] font-bold text-mint-600 uppercase tracking-widest">Actually Pre-Ordered</p>
                    <div className="flex items-baseline gap-2">
                       <span className="text-4xl font-black text-mint-600">{detail.preOrderedQuantity}</span>
            ...
            <div className="bg-slate-900 rounded-[32px] p-8 text-white">
               ...
               <ul className="space-y-3 text-sm text-slate-400">
                  <li className="flex items-start gap-3">
                     ...
                     <span>Ensure all items are physically verified before marking as <b>DONE</b>.</span>
                  </li>
                  <li className="flex items-start gap-3">
                     <div className="w-1.5 h-1.5 rounded-full bg-mint-400 mt-1.5 shrink-0" />
                     <span>The current batch has a fill rate of <b>{Math.round((detail.preOrderedQuantity / detail.targetQuantity) * 100)}%</b>.</span>
                  </li>
                  <li className="flex items-start gap-3">
                     <div className="w-1.5 h-1.5 rounded-full bg-mint-400 mt-1.5 shrink-0" />
                     <span>Marking as DONE will notify the system to update available stock for order fulfillment.</span>
                  </li>
               </ul>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
             ...
             <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                ...
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <p className="text-sm font-bold text-slate-800">{staffMap[detail.managerResponsibility] || 'Unknown Manager'}</p>
                ...
                <div className="space-y-4">
                   ...
                   <div className="pt-4 border-t border-slate-50">
                      <div className="flex justify-between items-center">
                         ...
                         <span className="px-3 py-1 bg-slate-50 rounded-lg text-xs font-black text-slate-800">{formatDate(detail.targetDate)}</span>
                      </div>
                   </div>
                </div>
             </div>
