...
} from '@/shared/types/product-create.types'

export const PRODUCT_CREATE_INPUT_CLASSNAME =
...
export function toBrandNullable(value: string): string | null {
  const v = value.trim()
  return v.length ? v : null
}

export function toStringArrayFromText(value: string): string[] {
  return value
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
...
export function buildVariant(v: ProductCreateFormState['variants'][number]): Variant {
  ...
  return {
    ...
    imgs: toStringArrayFromText(v.imgsText),
    isDefault: v.isDefault
  }
}

export function buildProductCreateRequest(state: ProductCreateFormState): ProductCreateRequest {
...
export function validateProductCreateStrict(req: ProductCreateRequest): string[] {
  const errors: string[] = []
